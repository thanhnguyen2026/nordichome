  ...
  return (
    <div className="flex min-h-screen bg-stone-50">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 h-14 bg-stone-900 text-amber-50 flex items-center justify-between px-4">
        <button onClick={() => setSidebarOpen(true)} aria-label="Mở menu" className="cursor-pointer">
          <Menu size={22} />
        </button>
      ...
      `}>
          ...
          {NAV.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                ...
                className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition mb-1 ${
                  active ? 'bg-stone-800 text-amber-100 font-semibold' : 'hover:bg-stone-800 hover:text-amber-100'
                }`}>
        ...
        <div className="p-4 border-t border-stone-800">
          ...
          <button onClick={handleLogout}
            className="flex items-center gap-1.5 w-full text-left text-xs text-stone-400 hover:text-red-400 transition cursor-pointer">
            <LogOut size={13} />
            Đăng xuất
          </button>
          <Link href="/" className="flex items-center gap-1.5 mt-2 text-xs text-stone-400 hover:text-amber-200 transition">
            <Home size={13} />
            Về trang chủ
          </Link>
        </div>
      ...
      {/* Main content */}
      <main className="flex-1 px-2 py-4 pt-20 md:p-8 md:pt-8 overflow-x-hidden min-w-0">
        {children}
      </main>
    </div>
  )
