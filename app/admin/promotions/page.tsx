...
import { Edit2, Trash2, Plus, X, PartyPopper, Save } from 'lucide-react'
...
export default function AdminPromotions() {
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirm('Xoá khuyến mãi này?', { danger: true }))) return
    ...
    <AdminLayout>
      ...
      <div className="flex items-center gap-3 mb-1">
        <div className="w-10 h-10 rounded-xl bg-stone-900 flex items-center justify-center flex-shrink-0">
          <PartyPopper size={18} className="text-amber-100" aria-hidden="true" />
        </div>
        <h1 className="text-2xl font-black leading-tight">Khuyến mãi</h1>
      </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex gap-2 mb-2">
            <button type="button" onClick={() => set('scope', 'all')}
              className={`flex-1 text-sm px-3 py-2 rounded-lg border font-semibold transition cursor-pointer ${
